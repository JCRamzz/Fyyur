...
def search_venues():
    ...
    for venue in query:
        data.append({
            "id": venue.id,
            "name": venue.name,
            "num_upcoming_shows": len(db.session.query(Shows).filter(Shows.venues_id == venue.id).filter(Shows.start_time > datetime.now()).all()),
